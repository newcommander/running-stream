...
const char program_name[] = "rtsp_server";
...
extern int pickup_pkt(AVPacket *pkt, int *stop);
extern void stop_and_clear(void);
...
static char *ctime1(char *buf2, int buf_size)
{
    ...
    if (gettimeofday(&tv, NULL) != 0) {
        snprintf(buf2, buf_size, "[00-00 00:00:00.000000]");
    } else {
        if (!localtime_r(&tv.tv_sec, &ti)) {
            snprintf(buf2, buf_size, "[11-11 11:11:11.111111]");
        } else {
            strftime(buf2, buf_size, "[%m-%d %H:%M:%S", &ti);
            snprintf(usec, 32, ".%06ld]", tv.tv_usec);
            strncat(buf2, usec, 32);
        }
    }
...
#endif
static void http_log1(const char *fmt, ...)
{
    va_list vargs;
    va_start(vargs, fmt);
    http_vlog(fmt, vargs);
...
#define http_log(fmt, arg...) \
    do { \
        http_log1("[%s:%d] " fmt, __FILE__, __LINE__, ## arg); \
    } while (0);
...
static int http_server(void)
{
    ...
    if (!rtsp_server_fd) {
        http_log("RTSP disabled.\n");
        av_free(poll_table);
        return -1;
    }

    http_log("RTSPserver started.\n");
...
static void close_connection(HTTPContext *c)
{
    ...
    if (c->fmt_in) {
        c->fmt_in->streams = NULL;
        c->fmt_in->nb_streams = 0;
    }
    ...
    for(i=0;i<nb_streams;i++) {
        ...
        if (ctx) {
            if (ctx->pb) {
                ctx->pb->write_packet = NULL; // aviod real writing action, which was found would cause seg_fault
            }
...
static int rtsp_set_http_context(HTTPContext *c)
{
    AVFormatContext *s = NULL;
    s = avformat_alloc_context();
    if (!s) {
        http_log("allocating format context failed\n");
        return -1;
    }

    s->probe_score = AVPROBE_SCORE_MAX;
    s->streams = config.first_stream->streams;
    s->nb_streams = config.first_stream->nb_streams;
    s->flags |= AVFMT_FLAG_GENPTS;
                           ...
                           RTSPMessageHeader *h)
{
 ...
 found:
    ...
    if (!rtp_c) {
        ...
        if (rtsp_set_http_context(rtp_c) < 0) {
            http_log("Setting http context failed\n");
            rtsp_reply_error(c, RTSP_STATUS_INTERNAL);
            return;
        }
...
int rtsp_main(int argc, char **argv, AVCodecContext *ctx_src, const char *config_str)
{
    ...
    if (!config_str) {
        http_log("Could not found config string");
        return 1;
    }
    config.filename = "";
    ...
    if ((ret = ffserver_parse_ffconfig(config.filename, &config, config_str)) < 0) {
        http_log("Error reading configuration file '%s': %s\n",
    ...
    codec = avcodec_find_encoder(ctx_src->codec_id);
    if (!codec) {
        http_log("canot find encoder: %s", avcodec_get_name(ctx_src->codec_id));
        return 1;
    }

    ctx = avcodec_alloc_context3(codec);
    if (!ctx) {
        http_log("could not allocate codec context for %s", avcodec_get_name(ctx_src->codec_id));
        return 1;
    }

    ctx->bit_rate = ctx_src->bit_rate;
    ctx->width = ctx_src->width;
    ctx->height = ctx_src->height;
    ctx->request_sample_fmt = ctx_src->request_sample_fmt;
    ctx->bits_per_coded_sample = ctx_src->bits_per_coded_sample;
    ctx->pix_fmt = ctx_src->pix_fmt;
    ctx->time_base = ctx_src->time_base;
    ctx->gop_size = ctx_src->gop_size;
    ctx->max_b_frames = ctx_src->max_b_frames;

    add_av_stream1(config.first_stream, ctx, 1);

    config.first_stream->streams[0]->time_base = (AVRational) {1, 30};
