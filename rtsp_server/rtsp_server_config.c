...
char *pos = NULL;
char *get_line(const char *config, char *buf, int buf_size)
{
    int config_len = strlen(config);
    if ((pos < config) || (pos > config + config_len)) {
        return NULL;
    }
    ...
    int copy_len = p - pos < buf_size ? p - pos : buf_size;
    memcpy(buf, pos, copy_len);
    buf[copy_len] = 0;
    pos = p + 1;
    return buf;
}

int ffserver_parse_ffconfig(const char *filename, FFServerConfig *config, const char *_config)
{
    FILE *f;
    char line[1024];
    char cmd[64];
    ...
    */
    ...
    pos = (char*)_config;
    while (get_line(_config, line, sizeof(line)) != NULL) {
        config->line_num++;
        p = line;
        while (av_isspace(*p))
            p++;
